...
export const iso13485Documents: DocTemplate[] = [
  {
    ...
    certTests: [
      "설계·개발(7.3) 적용/제외 여부 내부 결정 검토 — 제외 근거 충분성 확인",
    ],
    requiredData: [
      "조직 제품 목록·서비스 범위 (인증 대상 제품 확정)",
      "사업장 목록 (멀티사이트 해당 시 각 사이트 활동 목록)",
      "이전 인증 범위 문서 (갱신 시 비교용)",
  ...
  },
  {
    ...
    requiredData: [
      "경영진 품질 의지 확인 미팅 결과 (최고경영진 면담·결의문)",
      "조직 품질목표 초안 (방침과 연계용)",
      "이전 방침 문서 (갱신 시 개정 이력 유지용)",
  ...
  },
  {
    ...
    requiredData: [
      "직전 내부 심사 보고서 전체 (부적합·CAR 현황 포함)",
      "고객 불만 통계 (분기별 불만 건수·유형·처리 현황)",
      "CAPA 현황 요약 (미결 CAPA 목록 포함)",
      "제품 적합성 데이터 (불량률·검사 결과 트렌드)",
      "규제 변경 사항 모니터링 결과 (최근 분기)",
      "이전 경영 검토 후속 조치 이행 현황",
      "품질목표 달성 현황 데이터",
      "ISO 13485:2016 Clause 5.6.2 필수 입력 항목 체크리스트",
    ],
    checklist: [
  ...
  },
  {
    ...
    certTests: [
      "공급자 현장 심사 또는 서면 평가 완료 확인 — 영향 등급 H(High) 공급자 우선",
      "공급자 인증서 유효성 검토 — ISO 9001·의료기기 관련 인증 만료 여부 확인",
    ],
    requiredData: [
      "공급자 명단 초안 (내부 구매 기록·ERP 시스템 출력)",
      "원자재·서비스 영향 등급 분류 기준 문서",
      "공급자 인증서 사본 (ISO 9001 등 최신본)",
      "최근 2년 납품 이력 및 불량률 데이터",
      "공급자 평가 설문/체크리스트 양식",
      "ISO 13485:2016 Clause 7.4.1 원문",
    ],
    checklist: [
  ...
  },
  {
    ...
    certTests: [
      "심사원 자격 확인 — ISO 13485 내부 심사원 교육 이수 및 독립성 요건 충족 여부 검토",
      "심사 기준 문서 최신 버전 확인 — ISO 13485:2016, 관련 SOP, 적용 규제 요건 최신화",
    ],
    requiredData: [
      "연간 내부 심사 프로그램 및 일정표",
      "ISO 13485:2016 조항별 심사 체크리스트",
      "전년도 내부 심사 보고서 (비교 기준)",
      "CAR(시정 조치 요청) 처리 현황 대장",
      "심사 대상 프로세스별 SOP·작업지시서 최신본",
      "심사원 독립성 확인 서류 (자기 업무 제외 배정 근거)",
    ],
    checklist: [
  ...
  },
  {
    ...
    certTests: [
      "시정 조치 효과 검증 — 조치 완료 후 30~90일 내 동일 문제 재발 여부 추적 확인",
      "수평 전개(Lateral Spread) 적용 확인 — 유사 제품·프로세스에 동일 위험이 있는지 검토",
    ],
    requiredData: [
      "발원 문서 (내부 심사 보고서 CAR, 고객 불만 보고서, 바이질런스 보고 등)",
      "관련 SOP·작업지시서 최신본 (위반 절차 확인용)",
      "제품·프로세스 품질 데이터 (불량률 추이, 불만 이력)",
      "5Why·Fishbone 분석 워크시트 양식",
      "이전 유사 CAPA 이력 (반복 문제 여부 파악)",
      "ISO 13485:2016 §8.5.2/8.5.3 원문",
    ],
    checklist: [
