...
export function DocumentWorkspace() {
  ...
  // Select based on doc type (iso- prefix = ISO 13485 doc; IVDD/MDSAP fall through to ivdr slot)
  const { getStatus, setStatus } = isISODoc ? isoProgress : ivdrProgress;
  const status = doc ? getStatus(doc.id) : "not_started";
  const nextStatus = STATUS_NEXT[status];

  // 일정 — cert type에 맞는 마일스톤 날짜 로드
  ...
  const journeyLabel = isISODoc ? "ISO 13485 여정" : isIVDDDoc ? "IVDD 여정" : isMDSAPDoc ? "MDSAP 여정" : "IVDR 여정";
        ...
        <div style={{ marginBottom: "var(--s-6)" }}>
          ...
          <div className="flex flex-wrap items-center gap-2" style={{ marginBottom: "var(--s-3)" }}>
            ...
            {isIVDDDoc && (
              <span
                className="inline-flex items-center gap-1.5 rounded-full font-semibold text-text-on-color"
                style={{ background: "var(--p2)", fontSize: "var(--t-xs)", padding: "2px 10px" }}
              >
                IVDD
              </span>
            )}
            {isMDSAPDoc && (
              <span
                className="inline-flex items-center gap-1.5 rounded-full font-semibold text-text-on-color"
                style={{ background: "var(--p4)", fontSize: "var(--t-xs)", padding: "2px 10px" }}
              >
                MDSAP
              </span>
            )}
              ...
              {scheduleEntries.length > 0 && (() => {
                ...
                return (
                  <div
                    ...
                    style={{ borderColor: `var(${meta.colorVar})`, background: `var(${meta.tintVar})`, padding: "var(--s-4)" }}
                  >
                    <div className="flex items-center gap-2" style={{ marginBottom: "var(--s-3)" }}>
                      <CalendarDays size={16} style={{ color: `var(${meta.colorVar})` }} aria-hidden />
                      <h3 className="font-bold text-text" style={{ fontSize: "var(--t-sm)" }}>
                        {meta.label} 일정
                      </h3>
                      <Link
                        to="/schedule"
                        className="ml-auto font-semibold"
                        style={{ fontSize: "var(--t-xs)", color: `var(${meta.colorVar})` }}
                      >
                        편집 →
                      </Link>
                    </div>
                          ...
                          <li key={milestone.key} className="flex items-start gap-2">
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <div className="text-text-muted" style={{ fontSize: "var(--t-xs)" }}>
                                {date}
                                {!isPast && diffDays === 0 && (
                                  <span className="ml-1 font-bold" style={{ color: `var(${meta.colorVar})` }}>D-Day</span>
                                )}
                                {!isPast && diffDays > 0 && (
                                  <span className="ml-1 text-text-subtle">D-{diffDays}</span>
                                )}
